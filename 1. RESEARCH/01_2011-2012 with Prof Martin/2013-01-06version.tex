...
We have the corresponding analogues for $\delta_\pi(x; k, l_1, l_2)$, where the subscript is replaced by a different prime-counting function:
%
\begin{align*}
...
The primary tools are called the ``explicit formulas'', linking the functions $\pi(x; k, l)$ to the distribution of zeros of the Dirichlet-$L$ functions $L(s, \chi)$ in the critical strip, $0< \Re(s)<1$, for characters $\chi$ modulo $k$.

The asymptotic formula for $\pi(x; k, l)$ gives
\begin{equation}\label{asym}
\pi(x; k, 1)=\Pi(x; k, l)-\frac{N_k(l)}{\varphi(k)}\frac{x^{1/2}}{\log x}+o\left(\frac{x^{1/2}}{\log x} \right) \;\;\;\;\;(x \to \infty)
\end{equation}
...
\begin{align*}
\varphi(k)\Pi(x; k, l) &{}= \varphi(k)\left(\frac{\psi(x; k, l)}{\log x}+\int_2^x\frac{\psi(t; k, l)}{t \log^2 t}\,dt \right)\\
&= \sum_{\chi \in D_k}\bar{\chi}(l)\left(\frac{\Psi(x; \chi)}{\log x}+\int_2^x\frac{\Psi(t; \chi)}{t \log^2 t}\,dt \right)
\end{align*}
and by equation~\eqref{asym} we have:
\begin{align*}
\varphi(k)\delta_\pi(x; k, l_1, l_2)&=\varphi(k)\left(\frac{\delta_\psi(x; k, l_1, l_2)}{\log x}+\int_2^x\frac{\delta_\psi(t; k, l_1, l_2)}{t \log^2 t}\, dt \right)\\
&\qquad-\left(N_k(l_1)-N_k(l_2) \right)\frac{x^{1/2}}{\log x}+o\left(\frac{x^{1/2}}{\log x}\right)\\
&=\sum_{\chi \in C_k}\left(\bar{\chi}(l_1)-\bar{\chi}(l_2)\right)\left(\frac{\Psi(x; \chi)}{\log x}+\int_2^x\frac{\Psi(t; \chi)}{t \log^2 t}\,dt\right)\\
&\qquad-\left(N_k(l_1)-N_k(l_2) \right)\frac{x^{1/2}}{\log x}+o\left(\frac{x^{1/2}}{\log x}\right) \;\;\;\; (x \to \infty).
\end{align*}
...
\begin{align*}
e_1(x):&=e^x, \;\;\;\;\;\;\;\;\, e_\nu(x):=e_{\nu-1}\left(\exp(x)\right)\\
\log_1(x):&=\log x, \;\;\;\;\; \log_\nu(x):=\log_{\nu-1}\left(\log(x)\right)
\end{align*}
\end{dfn}
%% 1.1 of 1962.Knaposwki_2 
\begin{thm}[\cite{1962.Knaposwki_2} Theorem 1.1] For any $k$ in \eqref{II.2}
\begin{align*}
\max_{T^{{1/3}}\leq x \leq T}\delta_\psi(x; k, 1, l)>& \sqrt{T}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)\\
\min_{T^{{1/3}}\leq x \leq T}\delta_\psi(x; k, 1, l)<&- \sqrt{T}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)
\end{align*}
\end{thm}
...
\sigma \geq \tfrac{1}{2},\;\;\;\;\; |t|\leq \frac{c_2}{\log_3\left(k+e_3(1) \right)}, \;\;\;\;\;(s=\sigma +it)
\end{equation}
...
\beta_0\geq \tfrac{1}{2},\;\;\;\; \gamma_0 > 0,
\end{equation}
where $\varrho_0$ is a zero of an $L(s, \chi^*)$ belonging to modulo $k$ with $\chi^*(l)\neq 1$ and $T>\max\left(c_3, e_2(10|\varrho_0|)\right)$, then the inequalities 
\begin{align*}
\max_{T^{{1/3}}\leq x \leq T}\delta_\psi(x; k, 1, l)>& T^{\beta_0}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)\\
\min_{T^{1/3}\leq x \leq T}\delta_\psi(x; k, 1, l)< &-T^{\beta_0}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)
\end{align*}
hold.
\end{thm}
...
\begin{align*}
\max_{T^{1/3}\leq x \leq T}\delta_\Pi(x; k, 1, l)>& \sqrt{T}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)\\
\min_{T^{1/3}\leq x \leq T}\delta_\Pi(x; k, 1, l)< &- \sqrt{T}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)
\end{align*}
\end{thm}
...
Let $k$ be a number in~\eqref{II.2}. If $\varrho_0$ with~\eqref{II 1.4} holds is a zero of an $L(s, \chi^*)$ belonging to modulo $k$, $\chi^*(l)\neq 1$ and $T>\max\left(c_5, e_2(10|\varrho_0|)\right)$, then the inequalities 
\begin{align*}
\max_{T^{1/3}\leq x \leq T}\delta_\Pi(x; k, 1, l)>& T^{\beta_0}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)\\
\min_{T^{1/3}\leq x \leq T}\delta_\Pi(x; k, 1, l)<&-T^{\beta_0}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)
\end{align*}
hold.
\end{thm}
...
\begin{equation*}
T>\max\left(c_6, e_2(10|\varrho_0|), e_2(k), e_2\Big(\frac{1}{Z(k)^3}\Big)\right)
\end{equation*}
...
In the interval 
\[
0<x<\max \left(c_7, e_2(k), e_2\Big(\frac{1}{Z(k)^3} \Big) \right)
\]
...
Here
\begin{equation*}
c_7=\max\left(c_6, e_2(10(1+c_2))\right)
\end{equation*}
\end{thm}
...
\[
T>\exp\left( c_9 \left(\exp(k)+\exp\Big(\frac{1}{Z(k)^3} \Big) \right)^2\right)
\]
the inequalities
\begin{align*}
...
\begin{equation*}
T> \max\left(c_6, e_2(k), e_2\Big(\frac{1}{Z(k)^3}\Big)\right),
\end{equation*}
...
\begin{align}
\label{II.5.1.a}
\max_{\exp(\log_3^{1/130}T ) \leq x \leq T} \frac{\delta_\pi(x; k, 1, l)}{\left( \frac{\sqrt{x}}{\log x}\right)}  &> \frac{1}{100} \log_5 T\\
\label{II.5.1.b}
\min_{\exp(\log_3^{1/130}T ) \leq x \leq T} \frac{\delta_\pi(x; k, 1, l)}{\left( \frac{\sqrt{x}}{\log x}\right)}  &< - \frac{1}{100} \log_5 T
\end{align}
\end{thm}
...
\begin{equation} 
\label{II.5.2}
T>\max\left( e_5(c_{11} k), e_2\Big(\frac{1}{Z(k)^3}\Big)\right)
\end{equation}
...
\begin{equation*} 
1\leq x \leq \max \left( e_5(c_{11} k), e_2\Big(\frac{1}{Z(k)^3}\Big)\right)
\end{equation*}
...
\begin{equation*}
T>\max \left(e_4(k^{c_{14}}), e_2\Big(\frac{2}{Z(k)^3} \Big)\right)
\end{equation*}
...
\[
0 < x \leq \max  \left(e_4(k^{c_{14}}), e_2\Big(\frac{2}{Z(k)^3} \Big)\right)
\]
...
\[
T> \max\left(c_{15}, e_2(10|\varrho_0|)\right)
\] 
the inequalities 
\begin{align*}
\max_{T^{1/3}\leq x \leq T}\delta_\pi(x; k, 1, l)>& T^{\beta_0}\exp\left( -41\frac{\log(T)\log_3{(T)}}{\log_2{(T)}}\right)\\
...
\[
T>\max\left(c_{16}, e_2(k), e_2\Big(\frac{1}{Z(k)^3} \Big)\right),
\]
...
\begin{equation*}
T>\max \left( c_{16}, e_2(k), e_2\Big(\frac{1}{Z(k)^3} \Big), e_2(10|\varrho|) \right),
\end{equation*}
...
\[
T>\max\left(c_{18}, e_2(2k), e_2\Big(\frac{2}{Z(k)^3}\Big) \right)
\]
...
T>\max\Bigg\{e_2(c_{22}k^{20}), \exp\left( 2\exp\Big(\frac{1}{A(k)^3}\Big)+c_{22}k^{20}\right)\Bigg\}
\end{equation}
we have for $l_1\neq l_2$ the inequalities:
\begin{align*}
\max_{T^{{1/3}}\leq x \leq T}\delta_\psi(x; k, l_1, l_2)>& \sqrt{T}\exp\left(-44\frac{\log T \log_3 T}{\log_2 T}\right) \\
\max_{T^{{1/3}}\leq x \leq T}\delta_\Pi(x; k, l_1, l_2)>& \sqrt{T}\exp\left(-44\frac{\log T \log_3 T}{\log_2 T}\right)
\end{align*}
\end{thm}
...
For 
\begin{equation*}
T>\max\Bigg\{\exp\left(9\exp(2c_{22} k^{20}\right), \exp\left( 72\exp\Big(\frac{2}{A(k)^3}\Big)+18c_{22}^2k^{40}\right)\Bigg\}
\end{equation*} 
the inequalities 
\begin{align*}
...
\[
T>\max\Bigg\{e_2\left(c_{23}k^{20}\right), \exp\left(2\exp\left(\frac{1}{A(k)^3} \right)+c_{24}k^{40}\right)\Bigg\}
\] 
the inequality
\[
\max_{T^{{1/3}}\leq x \leq T}\delta_\pi(x; k, l_1, l_2)>\sqrt{T}\exp\left(-45\frac{\log{T}\log_3{T}}{\log_2{T}} \right)
\]
holds.
\end{rmk}
...
\begin{equation*}
T>\max\Bigg\{e_2(c_{22}k^{20}), \exp\left(2\exp\Big(\frac{1}{A(k)^3}\Big)+c_{23}k^{20}\right)\Bigg\}
\end{equation*}
...
\begin{equation}
T>\max\Bigg\{e_2(c_{24}k^{20}), \exp\left(2\exp\Big(\frac{1}{A(k)^3}\Big)+c_{24}k^{20}\right)\Bigg\}
\end{equation} 
...
\begin{equation*}
1\leq x \leq \max\left( e_2(k^{c_{25}}), e_2\Big( \frac{2}{Z(k)^3} \Big) \right)
\end{equation*}
...
only if
\begin{equation*}
\omega \geq \max \left( \exp(k^{c_{26}}), \exp\Big(\frac{2}{Z(k)^3}\Big)\right)
\end{equation*}
...
\Delta_\vartheta(x; 8, l_1, l_2)> \frac{1}{\sqrt\delta} \exp \left(-22\frac {\log\left( 1/\delta\right) \log_3\left( 1/\delta\right)}{\log_2\left( 1/\delta \right)} \right)
\end{equation}
holds unconditionally, and since $l_1$ and $l_2$ can be interchanged,
\begin{equation*}
\max_{\delta\leq x \leq \delta^{\frac 1 3}} 
\Delta_\vartheta(x; 8, l_1, l_2)<- \frac{1} {\sqrt\delta} \exp \left(-22\frac {\log\left( 1/\delta\right) \log_3( 1/\delta)} {\log_2\left( 1/\delta \right)} \right)
\end{equation*}
also holds.
\end{thm}
...
\begin{align*}
\max_{\delta\leq x \leq \delta^{\frac 1 3}}  \Delta_{\psi}(x; 8, 1, l)&>\frac{1}{\sqrt\delta}\exp\ \left(-22\frac {\log\left( 1/\delta\right) \log_3( 1/\delta)} {\log_2\left( 1/\delta \right)} \right) \\
\min_{\delta\leq x \leq \delta^{\frac 1 3}}  \Delta_{\psi}(x; 8, 1, l)&<-\frac{1}{\sqrt\delta}\exp\ \left(-22\frac {\log\left( 1/\delta\right) \log_3( 1/\delta)} {\log_2\left( 1/\delta \right)} \right)
\end{align*}
\end{thm}
...
\begin{equation*}
T>\max\left(c_{29}, e_2(k), \exp\Big(\frac{1}{Z(k)}\Big), e_2(|\varrho|)\right)
\end{equation*}
with suitable $U_1$, $U_2$, $U_3$, $U_4$ satisfying
\begin{align*}
T\exp\left(-\log^{11/12}T\right)\leq U_1 <U_2\leq T \\
T\exp\left(-\log^{11/12}T\right)\leq U_3 <U_4\leq T
\end{align*}
the inequalities 
\begin{align*}
\sum_{\substack{n\equiv 1 \mod k\\U_1\leq n \leq U_2}}\Lambda(n)-\sum_{\substack{n\equiv l \mod k\\U_1\leq n \leq U_2}}\Lambda(n) &\geq T^{\beta}\exp\left( -\log^{11/12}T\right)\\
\sum_{\substack{n\equiv 1 \mod k\\U_1\leq n \leq U_2}}\Lambda(n)-\sum_{\substack{n\equiv l \mod k\\U_3\leq n \leq U_4}}\Lambda(n) &\leq T^{\beta}\exp\left( -\log^{11/12}T\right)
\end{align*}
hold.
\end{thm}
...
\[
\lim_{x \to \infty}\sum_p\varepsilon(8; p, 1, l)\log(p)\exp\left(-\frac{p}{x}\right)=-\infty \;\;\;\;(l= 3, 5, 7)
\]
holds if and only if no $L(s, \chi)$ having $\mod 8$ with $\chi \in C_k$ vanishes for $\sigma > \h$ 
\end{xmp}
A few main results when $\exp\left(-\frac{p}{x}\right)$ is replaced by $\exp\left( -\frac{1}{r(x)}\log^2\left(\frac p x \right)\right)$ with suitable (``small'') $r(x)$:
...
\begin{equation*}
\lim_{x \to \infty} \sum_p{\varepsilon(k; p, l, 1) \log{p}\cdot  \exp\left( -\frac{1}{r(x)}\log^2\left(\tfrac{p}{x}\right)\right)} = +\infty
\end{equation*}
...
For any fixed $k$ satisfies the HC (Conjecture~\ref{HC}) and for all quadratic non-residues $l \mod k$, $(l, k) =1$, the relation
\begin{equation*}
\lim_{x \to \infty} \sum_p{\varepsilon(k; p, l, 1) \log{p} \cdot  \exp\left( -\frac{1}{r(x)}\log^2\Big(\frac p x \Big)\right)} = +\infty
\end{equation*}
...
the inequality
\begin{equation*}
\sum_p \varepsilon(k; p, l, 1) \log{p}  \exp \left( -\frac{1}{r(x)}\log^2\Big(\frac{p}{x} \Big)\right)> c_{31} \sqrt{x}
\end{equation*}
holds whenever $r_0< r \leq \log  x$ and $x > c_{32} k^{50}.$\\
As the contribution of primes $p$ with
\[
p>x\exp\left(10\sqrt{r\log x} \right)\;\;\;\;{\rm and}\;\;\;\;p<x\exp\left(-10\sqrt{r\log x} \right)
\]
is $o\left(\sqrt{x}\right)$, this theorem asserts under the given circumstances the preponderance of primes $\equiv l \mod k$ over those $\equiv 1 \mod k$ in the interval $\Big(x\exp(-10\sqrt{r\log x}), x\exp(10\sqrt{r\log x})\Big)$.
\end{thm}
...
L(\varrho_0, \chi) =0, \;\;\;\;\; \varrho_0=\beta + i\gamma, \;\;\;\;\; \beta>\tfrac{1}{2}, \;\;\;\;\; \gamma>0
\end{equation} 
then for all $T$ with 
\begin{equation}\label{IIb 5.5}
T>\max \left(c_{33}, \exp\Big( \pi^7E(k)^{-7}\Big), \exp\Big( \exp(k)\Big), \exp\Big( \left( \frac{4+\gamma^2}{\beta -\h}\right) ^{21}\Big) \right)
\end{equation}
...
such that 
\begin{align*}
\sum_{p}\varepsilon(k; p, l, 1) \log{p}\cdot  \exp\left( -\frac{1}{r_1}\log^2\Big(\frac p x_1 \Big)\right) &\geq T^{\beta}\exp \Big(-(1+\gamma^2)\log^{5/7}{T} \Big)\\
\sum_{p}\varepsilon(k; p, l, 1) \log{p}\cdot  \exp\left( -\frac{1}{r_2}\log^2\Big(\frac p x_2 \Big)\right) &\leq -T^{\beta}\exp \Big(-(1+\gamma^2)\log^{5/7}{T} \Big)
\end{align*}
...
T>\max\left(c_{37}, \exp\left(4\exp(3k)\right), \exp\Big(\frac{(20\pi)^6}{E(k)^6} \Big)\right)
\end{equation}
there exist $x_1$, $x_2$ in the interval 
\begin{equation*}
\Big( T\exp\left(-(\log T)^{5/6}\right), T\exp\left((\log{T})^{11/15}\right) \Big)
\end{equation*}
...
both the inequalities
\begin{align*}
\sum_p\varepsilon(k; p, l_2, l_1)\log{p}\exp\Big(-\frac{1}{\nu_1}\log^2\frac{p}{x_1} \Big) & > \sqrt{T}\exp\left(-c_{37}\log^{5/6}T\right)\\
\sum_p\varepsilon(k; p, l_2, l_1)\log{p}\exp\Big(-\frac{1}{\nu_2}\log^2\frac{p}{x_2} \Big) &<- \sqrt{T}\exp\left(-c_{37}\log^{5/6}T\right)
\end{align*}
hold.
\end{thm}
...
there exist for
\begin{equation*}
T>\max \left(c_{38}, \exp\Big( 4\exp(3k )\Big), \exp\Big( \frac{(20\pi)^6}{E(k)^6} \Big), \exp\Big(\exp(10|\varrho|)\Big) \right)
\end{equation*}
...
\begin{align*}
\sum_p\varepsilon(k; p, l_2, l_1)\log{p}\exp\Big(-\frac{1}{r_1}\log^2\frac{p}{x_1} \Big) & > T^{\beta}\exp\left(-c_{39}\log^{5/6}T\right)\\
\sum_p\varepsilon(k; p, l_2, l_1)\log{p}\exp\Big(-\frac{1}{r_2}\log^2\frac{p}{x_2} \Big) &<- T^{\beta}\exp\left(-c_{39}\log^{5/6}T\right)
\end{align*}
hold.
\end{thm}
...
such that
\begin{align*}
\sum_{U_1 \leq p \leq U_2} \varepsilon(k; p, 1, l)&>\sqrt{T}\exp\left(-c_{40}\log^{5/6}T \right)\\
\sum_{U_3 \leq p \leq U_4} \varepsilon(k; p, 1, l)&<-\sqrt{T}\exp\left(-c_{40}\log^{5/6}T \right)
\end{align*}
\end{thm}
...
\begin{equation*}
T>\max\left(c_{43}, \exp\Big( \frac{2}{\eta^4}\exp\left( \frac{1}{4}k^{10}\right)\Big) \right)
\end{equation*}
...
\begin{thm}[\cite{1965.Turan_5} Theorem]
 ...
 \begin{equation*}
 k>\max\left( c_{44}, \exp(\delta^{-20})\right)
 \end{equation*}
 ...
 then if
 \begin{equation*}
 a>\max\left( c_{45}, \exp(k\log^3k)\right)
 \end{equation*}
 and
 \begin{equation*}
 b=\exp\left(\log^2a \cdot(\log_2 a)^3  \right)
 \end{equation*}
...
\[
\lim_{x \to \infty}\sum_{p}(-1)^{(p-1)/2}\log p\cdot p^{-1/2}\exp\left({-{\log^2p}/{4x}}\right)=-\infty
\]
\end{thm}
which is a special care of:
\begin{thm}[\cite{1980.Besenfelder_2} Theorem]
\[
\lim_{x \to \infty}\sum_p (-1)^{{(p-1)}/{2}}\log{p}\cdot p^{-\alpha} \exp\left(- {\log^2 p}/{4x}\right)=-\infty\;\;\;\; {\rm for}\;\;\;\; 0 \leq \alpha \leq \h
\]
...
Unconditionally,
\[
\lim_{x \to \infty}\sum_{p}(-1)^{(p-2)/2}\log{p}\cdot{p^{-1/2}}\exp\left(-\log^2p/x \right)=-\infty 
\]
...
\]
The order of magnitude of divergence is given by
$\sim \h\sqrt{\pi x}\exp\left({\frac{x}{16}(1-2\alpha)^2}\right)
$\end{thm}
%%%
...
\[
\lim_{x\to \infty}\left(\left(\sum_{\substack{p \equiv 2 \mod 5\\{\rm or}\; p \equiv 3 \mod 5}}-\sum_{p \equiv 4 \mod 5}\right)\log p \cdot p^{-1/2}\exp(-\log^2 p/4x )\right) =+\infty
\]
\end{thm}
...
\begin{equation*}
\max_{\delta\leq x \leq \delta ^{{1/3}}} |\Delta_\pi(r; 8, l_1, l_2)| \geq \delta^{-1/2} \exp\left(\frac{23\log(1/\delta)\log_3(1/\delta)}{\log_2({1/\delta})}\right)
\end{equation*}
\end{thm}
...
\begin{equation*}
\max_{\delta\leq x \leq \delta ^{{1/3}}} |\Delta_\pi(r; k, 1, l)| \geq \delta^{-1/2} \exp\left(\frac{23\log(1/\delta)\log_3(1/\delta)}{\log_2({1/\delta})}\right)
\end{equation*}
\end{thm}
\vskip12pt
%%%%%%%%%%%%%
...
When $q$ s a natural number, we define $q^+:=\max\left(q, \exp(1260)\right)$, and assuming GRH (Conjecture~\ref{GRH}). Let $M(q)$ be the number of solution of the congruence $x^2 \equiv 1 \mod q$. Then there exists an $x$ with$x< e_2\left((q^+)^{170}+e^{18M(q)} \right)$ such that $\pi(x; q, 1)> \pi(x; q, a)$ for all $a \not\equiv 1 \mod q$.  Moreover, let $V(x)$ demote the number of sign changes of $\pi(t; q, 1)- \max_{a \not\equiv 1 \mod q}\pi(t; q, a)$ in the range $2 \leq t \leq q$, then
\[
V(x)>\frac{\log x}{\exp \left( (q^+)^{170}+e^{18M(q)}\right)}-1
\]
\end{thm}
...
then set
\[
\alpha(q, a):=\sum_{p|q}\frac{\log p}{\varphi(p^{\nu_p(q)})p^{l_p(a)}}\left(1-\frac{1}{p^{g_{q, p}}} \right)^{-1}
\]
...
\[
\delta(P_{k; a_1, \hdots, a_r})=\mu_{k; a_1, \hdots, a_r}\left(\{x \in \R^r: x_1>\cdots>x_r \}\right)
\]
...
\[
\exp\left(-c_{58}\sqrt{R}\exp(\sqrt{2\pi R}\,)\right)\leq \mu_{1;1}(B^\pm_R)\leq  \exp\left(-c_{59}\sqrt{R}\exp(\sqrt{2\pi R}\,)\right)
\]
...
\[
\delta_{k; a_1, \hdots, a_r}=2^{-(r-1)}\left(1+\sum_{\substack{B\subset\{1, \hdots, r-1\}\\B \neq \emptyset}}\left(\frac{i}{\pi} \right)^{|B|}\times {\rm P.V.}\int\cdots\int{\hat{\varrho}_{k; a_1, \hdots, a_r(B)}\prod_{j\in B}\frac{d\eta_j}{\eta_j}} \right)
\]
...
\begin{multline*}
\hat{\varrho}_{k; a_1, \hdots, a_r}(\eta_1, \hdots, \eta_{r-1})=\exp\left(\sum_{j=1}^{r-1}\left(c(k, a_j)-c(k, a_{j+1})\right)\eta_j \right)\\
\times\prod_{\substack{\chi \mod k\\ \chi \neq \chi_0}}F\left(\Bigg|\sum_{j=1}^{r-1}\left(\chi(a_j)-\chi(a_{j-1}) \right)\eta_j \Bigg|, \chi \right)
\end{multline*}
with
\[
F(z, \chi):=\prod_{\substack{\gamma>0\\L(\h+i\gamma, \chi)}}J_0\left(\frac{2z}{\sqrt{1/4+\gamma^2}}\right)
\]
and 
\[
...
\[
\mu_{q; a_1, \hdots, a_r}\Big(\|x\|>\lambda \sqrt{\Var(q)} \Big)=(2\pi)^{-r/2}\int_{\|x\|>\lambda}\exp\left( -\h \sum_{i=1}^r x_i^2\right)\,dx+O_r\left(\frac{1}{\log^2 q}\right)
\]
...
\[
\bigg|
\mu_{q; a_1, \hdots, a_r}\Big(\|x\|>\lambda\sqrt{\Var(q)} \Big)-(2\pi)^{-r/2}\int_{\| x\|>\lambda}\exp\left(-\h\sum_{i=1}^r x_i^2 \right)\,dx \bigg| \gg_r \frac{1}{\log^2 q}
\]
\end{thm}
...
\[
\exp\left(-c_{60}(r, A)\frac{V^2}{\varphi(q)\log q}\right)\ll \mu_{q; a_1, \hdots, a_r}(\|x\|>V)\ll \exp\left(-c_{61}(r, A)\frac{V^2}{\varphi(q)\log q}\right)
\]
...
\[\exp\left(-c_{63}(r)\frac{V^2}{\varphi(q)\log q}\exp\left(c_{65}(r)\frac{V}{\varphi(q)\log q}\right)\right) \ll \mu_{q; a_1, \hdots, a_r}(\|x\|>V)
\]
and
\[
\mu_{q; a_1, \hdots, a_r}(\|x\|>V) \ll \exp \left(-c_{62}(r)\frac{V^2}{\varphi(q)\log q}\exp\left(c_{64}(r)\frac{V}{\varphi(q)\log q} \right)\right)
\]
...
\[
\exp\left(-e^{L(q)}\sqrt{\frac{2(\varphi(q)-1)V}{\pi}} \exp\left(\sqrt{L(q)^2+\frac{2\pi V}{\varphi(q)-1}}\right)\left(1+O\left(\Big(\frac{\varphi(q)\log^2(q)}{V} \Big)^{1/4} \right) \right)\right),
\]
where 
\[
L(q)=\frac{\varphi(q)}{\varphi(q)-1}\left(\log q -\sum_{p|q}\frac{\log p}{p-1} \right) + A_0 -\log \pi,
\]
and 
\[
...
Lay, {\it Sign changes in Mertens' first and second theorems} (I have this preprint)

Look through Nathan Ng's work as well, and the \TeX\ file ``Wanted Articles!''

Lamzouri's work

Kisilevsky and Rubinstein, Chebotarev Sets (Acta Arith., 2015)

Lemke and Soundararajan, residue classes of consecutive primes (2016?)
...
%\begin{equation}
%T> \max\left(e_5(c_3k), e_2\Big(\frac{1}{Z(k)^3}\Big)\right)
%\end{equation}
