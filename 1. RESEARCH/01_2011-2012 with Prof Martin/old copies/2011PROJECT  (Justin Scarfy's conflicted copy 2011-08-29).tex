...
\fi
    ...
    \title{Comparative Prime-Number Theory:\\ 
    A Complete Survey}
...
%
\begin{equation}
w_f(T; k, l_1, l_2): = \sum_{\substack{\sgn\{\delta_f(x; k, l_1, l_2)\delta_f(x+1; k, l_1, l_2)\}=-1 \\  0<x\leq T}} 1 \;\;\;\;\;\;{\rm for}\;\;\;\;\;\; f=\pi, \psi, \vartheta, \Pi
\end{equation}
%
\begin{equation}
...
If for a $k$ that satisfies the Haselgrove Condition~\ref{HC} and
\begin{equation}
...
\begin{cnj}
[Lindel\"of Hypothesis(LH)]\label{LH}
\end{cnj}

\begin{cnj}
[Grand Simplicity Hypothesis (GSH)]\label{GSH}
\end{cnj}
...
In their series of papers published between 196? and 1962 (REF), Kna
%P 01
\begin{pbm}
...
If for a modulus $k$ Haselgrove Condition~\ref{HC}s holds for a $\rho_0$ with $\rho_0=\beta_0+i\gamma_0 $, then for 
\begin{equation}
...
$$
the functions $\delta_\psi(x; k, 1, l)$ and $\delta_\Pi(x; k, 1, l)$ certainly change their sign, when $k$ satisfies the Haselgrove Condition~\ref{HC}. 
...
If for a $k$ the Haselgrove Condition~\ref{HC} holds 
$$
...
Let $L(s, \chi^*)$ be an arbitrary $L$-function mod $k$ and (supposing Haselgrove Condition~\ref{HC} for $k$)
\begin{equation}
...
If Haselgrove Condition~\ref{HC} holds for a $k$ and
\begin{equation} 
\label{II.5.2}
...
If Haselgrove Condition~\ref{HC} holds for a $k$ then the interval 
\begin{equation} 
...
%5.4
\begin{thm}[II 5.4]
If Haselgrove Condtition holds for a $k$ and for $T$ with \ref{II.5.2}, then the inequalities
\begin{align}
...
If $k$ satisfies the Haselgrove Condition~\ref{HC} holds then for 
\begin{equation}
...
%III 1.3
\begin{thm}
If for a $k$ the Haselgrove Condition~\ref{HC} holds then in the interval
\[
...
%III 3.1
\begin{thm}[III 3.1]
If for a $k$ the Haselgrove Condition~\ref{HC} holds and $l$ satisfies \ref{x2 = l mod k} and \ref{x2 = 1 mod k} then for
\[
...
%III 3.2
\begin{thm}[III 3.2]
If for a $k$ the Haselgrove Condition~\ref{HC} holds and $l$ satisfies \ref{x2 = l mod k} and \ref{x2 = 1 mod k} , AND if further $\rho = \beta_0 + i\gamma$, $\beta_0 \geq \h$ is a zero for an $L(s, \chi)$ with $\chi(l) \neq 1$, then for 
\begin{equation}
...
%III 3.4
\begin{thm}[III 3.4]
If for a $k$ the Haselgrove Condition~\ref{HC} holds and 
\[
...
\begin{thm}
Let $k$ fulfill the Haselgrove Condition~\ref{HC}, $(l, k) =1$, and let $\rho=\beta+i\gamma$ be an zero of an $L(s, \chi)$ with $\chi(l)\neq 1$and $\beta\geq \h$. Then with a sufficiently large $c_3$ for 
\begin{equation}
...
\begin{thm} %1
For any fixed $k$ satisfies the Haselgrove Condition~\ref{HC} [REF] and for all quadratic non-residues $l \mod k$, $(l, k) =1$, the relation
\begin{equation}
...
\begin{thm} %IIb 3
Assume $E(k) \leq \frac{\sqrt k}{k}$, if for a k satisfying the Haselgrove Condition~\ref{HC} and a prescribed quadratic nonresidue $l$, no $L(s, \chi)$ with $\chi(l) \neq 1$ vanishes for $\sigma > \h$, then for suitable $c_4, c_5, c_6$ and
\[
...
\begin{thm} %IIb 4
If for a $k$ satisfying the Haselgrove Condition~\ref{HC} and a quadratic non-residue $l$ there exists an $L(s, \chi)$ with $\chi(k) \neq 1$ such that 
\begin{equation}
...
%6
\begin{thm}
For a $k$ satisfying the Haselgrove Condition~\ref{HC} and quadratic residue $l_1$ and quadratic non-residue $l_2$ mod $k$ with no $L(s, \chi)$ vanishes  for $\sigma>\h$ with $\chi(l_1)\neq \chi(l_2)$, then for sutible $c_4, c_5, C_6$ and
\begin{equation}
r_0=c_4\frac{\log k}{E(k)^2}
\end{equation}
...
Let $k$ satisfy the Haselgrove Condition~\ref{HC},
...
\subsection %1971
 ~\cite{1971.Stark}
Consider $\varphi(k)\pi(x, k, a)- \varphi(K)\pi(x, K, A)$, denote $\chi$ and $X$ the characters mod $k$ and $K$ respectively
$\chi_0$ and $X_0$ denote principle characters, whereas $\chi_R$ and $X_R$ denote real characters
Define
\begin{equation}
...
\begin{align}
A_T(u)&= A_T(u, k, a, K, A)\\
           &= \sum_{X\neq X_0}\sum_{\substack{\rho_X \\ \beta_X > 0, |\gamma_X |< T}} \frac{\overline X(A)}{\rho_X}\exp{(\rho_X-\h)u} - 
                \sum_{\chi \neq \chi_0} \sum_{\substack{\rho_\chi \\ \beta_\chi > 0, |\gamma_\chi |< T}} \frac{\bar\chi(A)}{\rho_\chi}\exp{(\rho_\chi-\h)u} \\
A_T^*(u)&= A_T^*(u, k, a, K, A)\\
           &= r+ \sum_{X\neq X_0}\sum_{\substack{\rho_X \\ \beta_X > 0, |\gamma_X |< T}} \frac{X(A)}{\rho_X}\exp{(\rho_X-\h)u} - 
                \sum_{\chi \neq \chi_0} \sum_{\substack{\rho_\chi \\ \beta_\chi > 0, |\gamma_\chi |< T}} \frac{\chi(A)}{\rho_\chi}\exp{(\rho_\chi-\h)u}
\end{align}
so the relation between them is simply
\begin{equation}
A_T^*(u)= r+\frac{1}{T}\int_0^T A_t(u) \, dt
\end{equation}
whenever the limit exists, we define
$A_\infty(u)=A_\infty(u; k; a; K, A) = \lim{T\to \infty}A_T(u; k, a; K, A)$
\begin{thm}
If none of $L(s, \chi)$ and $L(s, X)$ have real zeros strictly between $1/2$ and $1$ then for any $T>0$ and any $u$,
\begin{equation}
\lim_{y \to \infty}\sup{\frac{\varphi(k)\pi(x, k, a)- \varphi(K)\pi(x, K, A)}{\frac{\sqrt{y}}{\log y}}}\geq A_T^*(u)
\end{equation}
\end{thm}

\begin{thm}
Suppose that none of the functions $L(s, \chi)$ and $L(s, X)$ have real zeros in the range $0<s<1$
\begin{enumerate}
\item\label{1971.1} If $r(k, a; K, A) = 0$, then there is a constant $c>0$ such that
\begin{equation}
\lim_{y \to \infty}\sup{\frac{\varphi(k)\pi(x, k, a)- \varphi(K)\pi(x, K, A)}{\frac{\sqrt{y}}{\log y}}}\geq c
\end{equation}
\item If $r(k, a; K, A) > 0$, then the result of \ref{1971.1} is true with $c=r$
\end{enumerate}
\end{thm}
\subsection
~\cite{1972.Turan_7}
